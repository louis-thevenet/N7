...
Soit #espace_mesure et $(f_n)_(n in NN)$ une suite décroissante de fonctions mesurables positives. Soit $f = inf_(n in NN) f_n$
...
Justifier que l'intégrale de Lebesgue et de Riemann coincident et les calculer
=== 1. $sin(x)$ sur $[0, pi]$
#let interval = $[0, pi]$
...
=== 2. $e^(-x) cos(x)$ sur $RR_+$
#let interval = $RR_+$
$abs(f(x))<=_(f "continue") e^(-x) , forall x$  et $g : x |-> e^(-x)$ intégrable au sens de Riemann
...
= TD4
== Exercice 1
$ f : cases((E, cal(A)) times (E,cal(A)) -> (RR^+, cal(B)(RR^+)), (k,l) |-> u_(k,l))  $
...
$phi : cases(RR_+^* times RR_+^* -> RR_+^* times RR_+^*, (v,t) |-> (v/t, t^2))$ est un $cal(C)^1$-difféomorphisme sur $RR_+^* times RR_+^*$
...
- Domaine de continuité de $F$
    ...
    + $forall t in RR_+, forall x in RR_+ abs(f(x,t)) <= g(x) = 1/(1+x^2)$ positive intégrable
    Donc d'après le théorème de continuité sous le signe intégral, $F$ est continue sur $RR_+$
