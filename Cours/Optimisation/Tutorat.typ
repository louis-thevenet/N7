...
+ Déjà, on montre que $C$ est *fermé borné*
    ...
    - Fermé
        ...
        - $n$ $1/2$-espace fermés : $E_k = {x bar x_k >=0}$ (fermés comme images réciproques des projections $"pr"_k$ en $[0, +infinity[$)
        - un hyperplan affine $H={x in RR^n bar sum_k b_k = 1}$
    On peut conclure que $f$ admet au moins 1 min global sur le *compact* $C$ non vide ($b/norm(b)^2 in C$)

+ Est-ce que $C$ est convexe ?

    $H$ hyperplan est naturellement convexe

    $E_k$ est le volumme d'un côté de l'hyperplan $G_k={x bar x_k=0}$ est aussi naturelelment convexe
