...
#theorem[
  ...
  #definition[ Ainsi, l'EDP est dite :
...
#definition[ Pour $u:RR arrow RR²$, \ $cal(C)^2$ sur le segment $[ x-h_0, x+h_0]$, avec $h_0>0$.
  On a :
  $ exists C >= 0, forall h in ]0, h_0] | abs((u(x+h)-u(x))/h -u'(x)) <= C h $
  L'approximation est dite consistante d'ordre 1. ]
== Approximation centrée
#definition[
  Pour $u:bb(R) arrow bb(R)²$, \ $cal(C)^3$ sur le segment $[x-h_0, x+h_0]$, avec $h_0>0$.
  On a :
