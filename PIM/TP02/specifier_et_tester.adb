...
procedure Specifier_Et_Tester is
    ...
    function Pgcd (A, B : in Integer) return Integer with
        Pre =>  A > 0 and B > 0,
        Post =>  Pgcd'Result >= 1 -- c'est le résultat de la fonction
    ...
    procedure Utiliser_Pgcd is
        Resultat: Integer;
    begin
        Resultat := Pgcd (0, 10); -- 0 n'est pas >0, du coup ça termine pas
        Put ("Pgcd (0, 10) = ");
        Put (Resultat, 1);
        New_Line;
    end Utiliser_Pgcd;
    ...
    procedure Permuter (A, B: in out Integer) with
        Post => A = B'Old and B = A'Old -- les valeur d'origine
    is
        Copie_A : Integer;
    begin
        Copie_A := A;
