...
let ( >>= ) result f =
  match result with Success next -> f next | Failure -> Failure
...
let rec parseE stream =
  ...
  *)
  ...
  (* regle #1 *)
  | FunctionToken ->
      inject stream >>= accept FunctionToken >>= acceptIdent
      >>= accept BodyToken >>= parseE
  (* regle #2 *)
  | LetToken ->
      inject stream >>= accept LetToken >>= acceptIdent >>= accept EqualToken
      >>= parseE >>= accept InToken >>= parseE
  (* regle #3 *)
  | RecToken ->
      inject stream >>= accept RecToken >>= acceptIdent >>= accept EqualToken
      >>= parseE >>= accept InToken >>= parseE
  (* regle #4 *)
  | IfToken ->
      inject stream >>= parseE >>= accept ThenToken >>= parseE
      >>= accept ElseToken >>= parseE
  (* regle #5 *)
...
and parseEX stream =
  ...
  match peekAtFirstToken stream with
  (* regle #6 *)
  | EqualToken -> inject stream >>= accept EqualToken >>= parseER >>= parseEX
  (* regle #7 *)
  | RightParenthesisToken | EOSToken -> inject stream
  | _ -> Failure
...
and parseTX stream =
  ...
  match peekAtFirstToken stream with
  (* regle #9 *)
  | PlusToken -> inject stream >>= accept PlusToken >>= parseT >>= parseTX
  (* regle #10 *)
  | MinusToken -> inject stream >>= accept MinusToken >>= parseT >>= parseTX
  (* regle #11 *)
...
and parseFX stream =
  ...
  match peekAtFirstToken stream with
  (* regle 13 *)
  | TimesToken -> inject stream >>= accept TimesToken >>= parseF >>= parseFX
  (* regle 14 *)
  | DivideToken -> inject stream >>= accept DivideToken >>= parseF >>= parseFX
  (* regle 15 *)
  | RightParenthesisToken | EqualToken | PlusToken | MinusToken -> inject stream
  | EOSToken -> inject stream
  | _ -> Failure
...
and parseF stream =
  ...
  match peekAtFirstToken stream with
  (* regle 16 *)
  | MinusToken -> inject stream >>= accept MinusToken >>= parseF
  (* regle 17 *)
  | NumberToken _ -> inject stream >>= acceptNumber
  (* regle 20 *)
  | IdentToken _ | LeftParenthesisToken ->
      inject stream >>= parseFF >>= parseARG
  (* regle 21 *)
  | TrueToken -> inject stream >>= accept TrueToken
  (* regle 22 *)
  | FalseToken -> inject stream >>= accept FalseToken
  | _ -> Failure
...
and parseFF stream =
  ...
  (* regle 24 *)
  | IdentToken _ -> inject stream >>= acceptIdent
  | _ -> Failure
...
and parseARG stream =
  ...
  | LeftParenthesisToken ->
      ...
      >>= accept RightParenthesisToken
  (* regle 26 *)
  | RightParenthesisToken | EOSToken | TimesToken | PlusToken | MinusToken
  | DivideToken | ElseToken | ThenToken | InToken ->
      inject stream
  | _ -> Failure
