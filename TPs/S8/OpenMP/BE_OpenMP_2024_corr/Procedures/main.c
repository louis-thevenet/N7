...
int main(int argc, char **argv){
  ...
  t_start = usecs();
  #pragma omp parallel for private(s, procedure) //Correction
  for(p=0; p<np; p++){
    ...
    for(s=0; s<procedure.nsteps; s++){
      ...
      case PRINTER:
        ...
        #pragma omp critical (printer) //Correction (possible aussi avec des verrous (locks))
        use_printer(procedure, s);
        break;
      case CPU:
        /* printf("%3d  %2d -- Using CPU     %d\n",p,s,procedure.data[s]); */
        #pragma omp critical (cpu) //Correction
        use_cpu(procedure, s);
        break;
      case SCREEN:
