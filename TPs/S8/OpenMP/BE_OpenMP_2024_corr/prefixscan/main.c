...
void parallel_scan(int *x, int b, int n){
  ...
  #pragma omp parallel //Correction
  {
    #pragma omp single //Correction
    {
      /* Forward sweep */
      for(d=0; d<b; d++){
        for(i=0; i<n; i+=1<<d+1){
          l = i+(1<<d)-1;
          r = i+(1<<(d+1))-1;
          #pragma omp task depend(in:x[l]) depend(inout:x[r]) firstprivate(l,r) //Correction
          {
          x[r] = sum(x[l], x[r]);
        }
      }      
        /* Backward sweep */
        #pragma omp task depend(out:x[n-1]) //Correction
        { 
        x[n-1]=0;
        }
        ...
        for(d=b-1; d>=0; d--){
          for(i=0; i<n; i+=1<<(d+1)){
            l = i+(1<<d)-1;
            r = i+(1<<(d+1))-1;
            #pragma omp task depend(inout:x[l], x[r]) firstprivate(l,r) //Correction
            {
              t = x[l];
              x[l] = x[r];
              x[r] = sum(t, x[l]);
