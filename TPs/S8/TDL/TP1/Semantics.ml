...
and
  ...
  let leftvalue =
    ...
    (* Evaluation du paramètre gauche *)
  in
  ...
  | _ -> (
      ...
      | _ -> (
          ...
          | IntegerValue leftvalue, IntegerValue rightvalue -> (
              ...
              | Divide ->
                  if rightvalue = 0 then ErrorValue RuntimeError
                    (* Division par zéro *)
                  else IntegerValue (leftvalue / rightvalue)
              | _ -> ErrorValue TypeMismatchError)
          (* Vérification des types *)
...
and
  ...
  let value = value_of_expr _bvalue _env in
  match value with
  | ErrorValue _ as result -> result
  | _ -> value_of_expr _bin (_env @ [ (_ident, value) ])
...
and
  ...
  | BooleanValue b -> value_of_expr (if b then _bthen else _belse) _env
  | ErrorValue _ as result -> result
  | _ -> ErrorValue TypeMismatchError
...
and
    ...
    ruleCallByName _env _fexprc _pexpr =
...
and
  ...
  (* Appel par valeur *)
  let value = value_of_expr _pexpr _env in
  match value with
  | ErrorValue _ as result -> result
  | _ -> value_of_expr _fexpr _env
