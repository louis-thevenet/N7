...
let rec lookforMem (name : string) (env : (string * valueType) list) =
  match env with
  | [] -> NotFound
  | (key, value) :: others ->
  ...
  | _ -> (
      ...
      | _ -> (
          ...
          | IntegerValue leftvalue, IntegerValue rightvalue -> (
              ...
              | Divide ->
                  if rightvalue = 0 then (ErrorValue RuntimeError, leftmem)
                  else (IntegerValue (leftvalue / rightvalue), leftmem)
...
and
  ...
  match cval with
  | BooleanValue rcond ->
      if rcond then value_of_expr (bthen, cmem) env
      else value_of_expr (belse, cmem) env
...
and
    ...
    (* .............................................................................*)
    ruleInteger value mem =
  (IntegerValue value, mem)
...
and
  ...
  let pval, pmem = value_of_expr (expr, mem) env in
  match pval with
  | ErrorValue _ as result -> (result, pmem)
  | ReferenceValue s -> (
      match lookforMem s pmem with
      | Found v -> (v, pmem)
      | NotFound -> (ErrorValue (UnknownReferenceError s), pmem))
...
and
    ...
    (*      -> (ValueType * memory)                                                 *)
    (* .............................................................................*)
    ruleWrite env refexpr valexpr (mem : (string * valueType) list) =
  let val_val, mem1 = value_of_expr (valexpr, mem) env in
  match val_val with
  | ErrorValue _ as result -> (result, mem1)
  | _ -> (
      let ref_val, mem2 = value_of_expr (refexpr, mem1) env in
      match ref_val with
      | ErrorValue _ as result -> (result, mem2)
      | ReferenceValue s -> (
          match lookforMem s mem2 with
          | Found _ -> (NullValue, (s, val_val) :: mem2)
          | NotFound -> (ErrorValue (UnknownReferenceError s), mem2))
...
and
  ...
  let left_val, mem1 = value_of_expr (left, mem) env in
  match left_val with
  | ErrorValue _ as result -> (result, mem1)
  | NullValue -> (
      let right_val, mem2 = value_of_expr (right, mem1) env in
...
and
  ...
  let cond_val, mem1 = value_of_expr (cond, mem) env in
  match cond_val with
  | ErrorValue _ as result -> (result, mem1)
  | BooleanValue v ->
      if v then
        let body_val, mem2 = value_of_expr (body, mem1) env in
        match body_val with
        | NullValue -> ruleWhile env cond body mem2
        | _ -> (ErrorValue TypeMismatchError, mem2)
      else (NullValue, mem1)
  | _ -> (ErrorValue TypeMismatchError, mem1)
...
and
  ...
  let expr_val, mem1 = value_of_expr (expr, mem) env in
  match expr_val with
  | ErrorValue _ as result -> (result, mem1)
  | _ ->
      let ad = newReference () in
      (ReferenceValue ad, (ad, expr_val) :: mem1)
