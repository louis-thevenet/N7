...
let rec type_of_expr expr env =
  ...
  and
    ...
    print_endline (string_of_type tfct);
    match tfct with
      ...
      | _ -> print_endline (string_of_type tfct); ErrorType
  ...
  and
    ...
    let tright = type_of_expr right env in
    match tleft with
    | ReferenceType(tval) -> let (_, res) = unify tright tval in
      if res then 
        UnitType
      else
        ErrorType
