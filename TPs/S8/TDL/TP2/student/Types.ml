...
let rec normalize t =
  ...
  | FunctionType (tp, tr) ->
      ...
      if ntp <> ErrorType && ntr <> ErrorType then FunctionType (ntp, ntr)
      else ErrorType
  | ReferenceType t ->
      let nt = normalize t in
      if nt <> ErrorType then ReferenceType nt else ErrorType
...
and
  ...
  let tbody = type_of_expr body ((par, tpar) :: env) in
  FunctionType (tpar, tbody)

and
    (* ...............A COMPLETER .......................................*)
    ruleCall env fct par =
  let tfct = type_of_expr fct env in
  let tpar = type_of_expr par env in
  match tfct with
  | FunctionType (tin, tout) ->
      let _, res = unify tin tpar in
      if res then tout else ErrorType
  | VariableType (type_ref, _) -> (
      match !type_ref with
      | FunctionType (tin, tout) ->
          let _, res = unify tin tpar in
          if res then tout else ErrorType
      | UnknownType ->
          let new_var = newVariable () in
          let _, ures = unify tfct (FunctionType (tpar, new_var)) in
          if ures then new_var else ErrorType
      | _ -> ErrorType)
  | _ -> ErrorType
...
and
  ...
  if ur then type_of_expr bin ((ident, typeident) :: env) else ErrorType
...
and
    ...
    ruleRead env expr =
  let texpr = type_of_expr expr env in
  match texpr with ReferenceType tval -> tval | _ -> ErrorType

and
    (* ...............A COMPLETER .......................................*)
    ruleWrite env left right =
  let tleft = type_of_expr left env in
  let tright = type_of_expr right env in
  match tleft with
  | ReferenceType tval ->
      let _, res = unify tright tval in
      if res then UnitType else ErrorType
  | _ -> ErrorType
...
and
  ...
  let tleft = type_of_expr left env in
  let tright = type_of_expr right env in
  match tleft with UnitType -> tright | _ -> ErrorType

and
    (* ...............A COMPLETER .......................................*)
    ruleWhile env cond body =
  let tcond = type_of_expr cond env in
  let tbody = type_of_expr body env in
  match tcond with
  | BooleanType -> ( match tbody with UnitType -> UnitType | _ -> ErrorType)
  | _ -> ErrorType
