...
/**
 * Interface to mark a node in the Abstract Syntax Tree as a Scope in the
 * language.
 * 
 * @author Marc Pantel
 *
...
public interface Scope<D extends Declaration> {
	 ...
	 * - an option contained the first value associated with the name if the name is
	 * contained in the scope.
	 * 
	 * @param _name : Name of the element looked for in the scope.
	 * @return : An optional object that carry an element named _name if there
	 *         exists one in the scope, empty if not.
	 */
	 ...
	 * @return : True if the _declaration can be registered in the scope, false if
	 *         not.
	 */
