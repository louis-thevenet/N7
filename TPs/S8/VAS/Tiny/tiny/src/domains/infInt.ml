...
let to_int = function
  | Fin x
    when Int64.compare x (Int64.of_int min_int) >= 0
         && Int64.compare x (Int64.of_int max_int) <= 0 ->
      Some (Int64.to_int x)
  | _ -> None
...
let mul_lb x y =
  ...
  | (Fin x, Infty_pos | Infty_pos, Fin x) when Int64.compare x Int64.zero > 0 ->
      Infty_pos
  | (Fin x, Infty_neg | Infty_neg, Fin x) when Int64.compare x Int64.zero < 0 ->
      Infty_pos
  | _ -> Infty_neg
...
let div_lb x y =
  match (x, y) with
  | Fin x, Fin y -> (
      try Fin (Int64.div x y) with Division_by_zero -> Infty_neg)
