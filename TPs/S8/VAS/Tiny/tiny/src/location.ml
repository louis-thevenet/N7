...
let fprint ff loc =
  if loc.loc_line = loc.loc_line_end then
    Format.fprintf ff "File \"%s\", line %d, characters %d-%d:@ " loc.loc_file
      loc.loc_line (loc.loc_start + 1) loc.loc_end
  else
    Format.fprintf ff "File \"%s\", lines %d-%d, characters %d-%d:@ "
      loc.loc_file loc.loc_line loc.loc_line_end (loc.loc_start + 1)
      loc.loc_line_end_c
