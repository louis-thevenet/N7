...
module MakeRelational (D : Domain) : Relational.Domain = struct
  ...
  let fprint ff t =
    ...
    | Env m ->
        if Name.Map.is_empty m then Format.fprintf ff "⊤"
        else
          ...
          Name.Map.iter
            (fun n v ->
              if !first then (
                ...
                first := false)
              else Format.fprintf ff ",@ %s : %a" n D.fprint v)
            m;
          Format.fprintf ff " }@]"
        ...
        let m =
          Name.Map.fold
            (fun n v1 m ->
              ...
              let v = f v1 v2 in
              Report.nlogf 5 "%a@ %s %a@ = %a." D.fprint v1 s D.fprint v2
                D.fprint v;
              if order_print 6 D.top v then m else Name.Map.add n v m)
            m1 Name.Map.empty
        in
        Env m
  ...
  let rec backeval_expr env e t =
    ...
    | Ast.Binop (_, bop, e1, e2) ->
        ...
        Report.nlogf 5 "%s@ %a@ %a@ %a@ = @[%a,@ %a@]." s D.fprint t1 D.fprint
          t2 D.fprint t D.fprint t1' D.fprint t2';
        backeval_expr (backeval_expr env e1 t1') e2 t2'
    | Ast.Rand _ ->
        let t' = eval_expr env e in
        let t = meet_print t t' in
        if order_print 6 t D.bottom then raise Exit else env
  ...
  let guard e t =
    ...
    | Env m -> (
        ...
        try Env (backeval_expr m e t') with Exit -> EnvBot)
end
