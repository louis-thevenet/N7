...
module Make (Dom : Relational.Domain) = struct
  let alarms m t =
    ...
    let rec alarms_stm env s =
      ...
      if Dom.order env bottom then (
        ...
        match s with
        | Ast.Asn (l, _, e) ->
            (alarms_expr env e, find_or_top (Location.end_p l))
        | Ast.Seq (_, s1, s2) ->
  ...
  let print m t output_filename =
    ...
    let rec fprint_stm ff s =
      ...
      match s with
      | Ast.Asn (l, n, e) ->
          Format.fprintf ff "%s = @[%a@];%a" n Ast.fprint_expr e fprint_annot
            (Location.end_p l)
      | Ast.Seq (_, s1, s2) ->
          Format.fprintf ff "@[<v>%a@ %a@]" fprint_stm s1 fprint_stm s2
      | Ast.Ite (l, e, s1, s2) ->
          Format.fprintf ff
            "@[<v>@[<v 2>if (@[%a@] > 0) {%a@ %a@]@ @[<v 2>} else {%a@ %a@]@ \
             }%a@]"
            Ast.fprint_expr e fprint_annot
            (Location.beg_p (Ast.loc_of_stm s1))
            fprint_stm s1 fprint_annot
            (Location.beg_p (Ast.loc_of_stm s2))
            fprint_stm s2 fprint_annot (Location.end_p l)
      | Ast.While (l, e, s) ->
          Format.fprintf ff "@[<v>%a@[<v 2>while (@[%a@] > 0) {%a@ %a@]@ }%a@]"
            ...
            fprint_stm s fprint_annot (Location.end_p l)
    in
