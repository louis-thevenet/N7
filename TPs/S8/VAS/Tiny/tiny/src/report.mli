...
(** Module providing functions to print errors in a way the compilation mode of
    Emacs understands while handling verbosity level and locations. *)

exception Error
(** Exception raised after printing an error message. *)

val verbosity : int ref
(** Verbosity level. Defines the amount of logging messages outputed. Default is
    1. 0 means quiet (only warning and error messages get printed). *)

val nlogf : int -> ('a, Format.formatter, unit, unit) format4 -> 'a
(** [nflogf n ff f <args>] prints a log message like [Format.fprintf] if [n] is
    greater or equal to [verbosity].
